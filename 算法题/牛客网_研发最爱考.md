...
* 链表
    ...
    * [6. 反转链表](https://www.nowcoder.com/practice/75e878df47f24fdc9dc3e400ec6058ca?tpId=190&&tqId=35203&rp=1&ru=/ta/job-code-high-rd&qru=/ta/job-code-high-rd/question-ranking)
    * [7. 设计LRU缓存结构](https://www.nowcoder.com/practice/e3769a5f49894d49b871c09cadd13a61?tpId=190&&tqId=35214&rp=1&ru=/ta/job-code-high-rd&qru=/ta/job-code-high-rd/question-ranking)
* 动态规划
...
public:
    ListNode* ReverseList(ListNode* pHead) {
        ListNode *pre = nullptr, *cur = pHead, *cur_next;
        while(cur) {
            cur_next = cur->next;
            cur->next = pre;
            pre = cur;
...
关键在于: 如何表示最近使用的元素和最近最久未使用的元素。

可以用一个双向链表来表示: 表尾是最近使用的元素，表头是最近最久未使用的元素。并且用unordered_map来记录cache中所有的元素，实现O(1)的查找。

set(key, value)的过程为: 

1. 在unordered_map中查找key
    1. 如果找到key，从key对应的结点从双向链表中移除，然后修改对应的value，重新插入到链表尾部
    2. 如果没有找到key，我们要向cache添加新的元素了，这个时候要看cache是否已满
        1. 如果cache已经满了，删除双向链表头部的元素(对应最近最久未使用的元素)，然后插入新的元素
        2. 如果cache未满，直接插入新的元素即可
...
public:
    ...
    void set(int key, int val) {
        if(cache.find(key) == cache.end()) {
            if(count == size) {
                // 如果cache满，要移除最近最久未使用的结点
                Node *delNode = head->next;
                deleteNode(delNode);
                // 删除cache
                auto it = cache.find(delNode->key);
                cache.erase(it);
                // 删除结点
                delete delNode;
                delNode = nullptr;
